...
/* 创建一个调用公众号api的客户端实例 */
ThirdpartyServer.prototype.createClient = function( user ){

	return new Client(
		this
		, user
		, function( callback ){ callback( null , { accessToken: user.access_token, expireTime: user.access_token_expires_in.getTime() }); }
...
/* 用户授权后的回调页面可以拿到auth_code，以其获取公众号信息 */
	...
	this.secrets( function(secret){
		...
		query.post(
			...
			function( err , auth_info ){
				...
				// 通过authorizer_access_token拉取公众号信息
				query.post(
...
ThirdpartyServer.prototype.get_user_info = function( wx_token , callback ){
	...
	that.models.read.WxUserInfo.find({ wx_token : wx_token },1,function(err , user){
		...
		var expires = user[0].access_token_expires_in;

		// 检查token是否还有效，如果无效重新刷新
		if( !expires || isNaN(new Date(expires).getTime()) || expires < new Date() ){

			return that.refresh_token( user[0].appid , user[0].refresh_token , function( err , token_info ){
				...
				user[0].access_token_expires_in = new Date(new Date().getTime() + token_info.expires_in*1000);
...
/* 通过authorizer_refresh_token 刷新 authorizer_access_token */
ThirdpartyServer.prototype.refresh_token = function( appid , refresh_token , callback ){
	...
	this.secrets( function(secret){
		...
		query.post(
			'https://api.weixin.qq.com/cgi-bin/component/api_authorizer_token?component_access_token='+ secret.access_token,
			{
				"component_appid":that.component_appid,
...
/* 从数据库中获取 component_access_token, ticket , pre_auth_code */
